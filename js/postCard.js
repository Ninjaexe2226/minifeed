function makePostElement(post, { state, onDelete, onUsernameClick }) {
  // 🌟 MODIFICACIÓN AQUÍ: El botón aparece si el post es propio O si el usuario actual es Admin
  ...
  // Detectamos si el post tiene una URL de avatar en la relación 'profiles'
  ...
  el.querySelector(".js-delete")?.addEventListener("click", async (e) => {
    ...
    // Un mensaje de confirmación amigable para el moderador
    const confirmMessage = isOwn 
      ? "Delete this post?" 
      : `🛡️ Admin Mode: Are you sure you want to delete @${username}'s post?`;
