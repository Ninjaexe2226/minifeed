...
function makeProfileLoaders(state, dom, { onUsernameClick, onDelete }) {
  ...
  return {
    ...
    following: async () => {
      ...
      // Consultamos a quiénes sigue este perfil
      const { data, error } = await sb
        .from('follows')
      ...
      data.forEach(item => {
        ...
        // Al hacer clic en cualquier parte de la fila, navega a su perfil
    ...
    settings: async () => {
      ...
      // 1. Renderizamos la interfaz con la opción de subir foto y cambiar bio
      dom.tabContents.settings.innerHTML = `
        <div style="max-width: 450px; margin: 0 auto; text-align: left; padding: 1rem;">
      ...
      // 2. Mapeamos los elementos del DOM dinámicos
      ...
      // 3. Traemos los datos actuales de Supabase (Bio y Avatar URL)
      const { data, error } = await sb
        .from('profiles')
      ...
      // 4. Escuchamos el evento de guardar
  ...
  try {
    ...
    // 1. ¿El usuario seleccionó una nueva imagen?
    const file = dom.avatarInput?.files[0];
    if (file) {
      // Creamos un nombre único para el archivo basado en el ID del usuario y el tiempo
      const fileExt = file.name.split('.').pop();
      const fileName = `${state.currentUser.id}-${Date.now()}.${fileExt}`;
      const filePath = `${fileName}`;

      // Subimos la imagen física al Bucket 'avatars' de Supabase Storage
      ...
      // Obtenemos la URL pública para poder visualizarla en la app
    ...
    // 2. Preparamos los datos que vamos a actualizar en la tabla profiles
    const updates = { bio: newBio };
    if (finalAvatarUrl) {
      updates.avatar_url = finalAvatarUrl;
    }

    // 3. Guardamos los cambios en la base de datos de Supabase
    ...
    // 4. Actualizamos la interfaz del perfil en tiempo real si los elementos existen
    if (dom.profileBio) dom.profileBio.textContent = newBio || 'No bio yet';
    if (finalAvatarUrl && dom.avatarPreview) {
