...
async function viewProfile(state, dom, userId, username, loaders, onFollowToggle) {
  ...
  // Guardamos temporalmente la función onFollowToggle en el estado para usarla en el header
...
async function loadProfileHeader(state, dom, userId, fallbackUsername) {
  ...
  // 🌟 LOGICA DEL BOTÓN EN EL PERFIL 🌟
  // Eliminamos cualquier contenedor anterior para que no se duplique al re-entrar
  ...
  if (!isMe && state.currentUser) {
    // Verificamos si ya lo seguimos mirando en state.followingIds
    const isFollowing = state.followingIds && state.followingIds.includes(userId);

    // Creamos un contenedor elegante justo debajo del handle
    ...
    // Lo insertamos justo antes de la biografía
    dom.profileBio.parentNode.insertBefore(followContainer, dom.profileBio);

    // Añadimos el evento click al botón del perfil
    followContainer.querySelector(".follow-btn")?.addEventListener("click", async (e) => {
      ...
      // Usamos el handler que pasamos desde el archivo principal
      ...
      if (success) {
        ...
        // Actualizamos los contadores de seguidores del perfil automáticamente
        await loadProfileCounts(dom, userId);
      }
      btn.disabled = false;
    });
